...
void LoadCascade(DetectionScanner& ds)
{
    ...
    filenames.push_back(g_model);//"/home/rover/catkin_ws/src/hdetection/src/combined.txt.model"
    types.push_back(NodeDetector::CD_HIK); // second node
    upper_bounds.push_back(353);
    filenames.push_back(g_model_);//"/home/rover/catkin_ws/src/hdetection/src/combined.txt.model_"
...
CRect PDetection(cv::Mat src, float fx){
    ...
    if (!((LResult.top-margin)>0))
    {
        bresults = src(cv::Rect(LResult.left,0,(LResult.right - LResult.left),(LResult.bottom - LResult.top)));
    }
    else
    {
        bresults = src(cv::Rect(LResult.left,LResult.top-margin,(LResult.right - LResult.left),(LResult.bottom - LResult.top)));
    }
    return LResult;
}    
...
{    
    ...
    ImageConvertor():it_(nh_){     
        ...
        roi_pub = nh_.advertise<struck::roi_input>("figure_roi",5);  
        track_sub = nh_.subscribe("track_output",5,&ImageConvertor::trackCallback,this);   
    }    
    
    ~ImageConvertor()    
    {        
    }    
    void trackCallback(const struck::track_output msg)
    {
        std::cout<<"track";
    }
    ...
    {    
        ...
        cv::Mat img_out;
        CRect RoiRect;
        RoiRect = PDetection(cv_ptr->image,fx);


        //RoiRect is Rect, you can send this as a message, but you need delete the image_transport::Publisher image_pub_ and add a ros::Publisher

        //publish the iamge
        sensor_msgs::ImagePtr msgnew;
        msgnew = cv_bridge::CvImage(std_msgs::Header(), "bgr8", bresults).toImageMsg();  
        image_pub_.publish(msgnew);  
        struck::roi_input roi_input_msg;
        roi_input_msg.pose1.x = RoiRect.left;
        roi_input_msg.pose1.y = RoiRect.top;
        roi_input_msg.pose2.x = RoiRect.right;
        roi_input_msg.pose2.y = RoiRect.bottom;
        roi_pub.publish(roi_input_msg);
    }    
